...
* **幻觉阻断机制**：严格的 Prompt 引擎与知识库混合检索策略（关键词+向量），有效遏制大模型在濒危语言上的“胡编乱造”。
...
3.  运行脚本：`python dynamic_data_process.py`
    *(注：脚本内置了严格的 Prompt，强制将数据提取为“藏语|木雅语|音标|中文|英文”的五列标准格式，并保留原有注脚符号。
     提取规则：
...
1.  在服务器终端启动 RAGFlow 服务：
    ...
    bash /root/start.sh
    ```
2.  通过浏览器访问服务地址并登录（默认账号：`admin@admin.com`，密码：`admin`）。
3.  **建立知识库**：
